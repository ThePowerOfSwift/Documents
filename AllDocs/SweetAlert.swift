...
open class SweetAlert: UIViewController {
    ...
    fileprivate func textFieldView() {
        ...
        textField?.placeholder = "Введите имя для папки"
    ...
    fileprivate func resizeAndRelayout() {
        ...
        // подвинуть, если клава ВКЛ
        contentView.frame = CGRect(x: (mainScreenBounds.size.width - kContentWidth) / 2.0, y: ((mainScreenBounds.size.height - y) / 2.0) - 100, width: kContentWidth, height: y)
    ...
    func closeAlert(_ buttonIndex:Int){
        if userAction !=  nil {
            ...
            if textField != nil && textField?.text?.isEmpty == false && isOtherButton {
               print(textField!.text!)

                let first = First()
                first.name = textField!.text!
                first.date = NSDate()
                first.type = type(.folder)
                first.priority = 0
                CoreDataManager.instance.saveContext()
                postNotification("reloadMain")
            }
    ...
    open func showAlertWithTxtFld(_ title: String, subTitle: String?, style: AlertStyle,buttonTitle: String,buttonColor: UIColor,otherButtonTitle:
        String?, action: ((_ isOtherButton: Bool) -> Void)? = nil) -> SweetAlert {
        self.showAlert(title, subTitle: subTitle, style: style, buttonTitle: buttonTitle,buttonColor: buttonColor,otherButtonTitle:
            otherButtonTitle,otherButtonColor: UIColor.red)
        textFieldView()
